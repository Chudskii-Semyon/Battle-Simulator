import { Helper, Adapter } from 'casbin';
...
export class NodeRedisAdapter {
  ...
  async loadPolicy(model) {
    return await this.redisInstance.get('policies', async (err, policies) => {
      var AdapterRef = this;
      ...
      if (!policies) {
        ...
        await this.redisInstance.SET('policies', JSON.stringify(tempPolicies), (err, reply) => {
          if (!err) {
